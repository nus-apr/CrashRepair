...
time_info = {
    ...
    definitions.KEY_DURATION_CONCRETE: '0',
...
def run(arg_list):
    ...
    time_check = time.time()
    taint_byte_list, taint_memory_list, taint_symbolic, cfc_info, taint_concrete = analyzer.analyze()
    duration = format((time.time() - time_check) / 60, '.3f')
    ...
    time_check = time.time()
    localizer.fix_localization(taint_byte_list, taint_memory_list, taint_symbolic, cfc_info, taint_concrete)
    duration = format((time.time() - time_check) / 60, '.3f')
